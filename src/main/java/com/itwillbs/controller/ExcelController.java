...
public class ExcelController {
	...
	public void orderList(HttpServletRequest request, HttpServletResponse response, Model model, PageDTO pageDTO) throws Exception {
		...
		List<OrderDTO> orderList = employeeService.getOrderListEx(); //수주목록 가져오기

		// 워크북 생성 : Excel 파일을 생성하는 객체
		Workbook o_wb = new HSSFWorkbook(); // HSSWorkbook은 XLS 형식
		Sheet o_sheet = o_wb.createSheet("수주 목록"); // 워크북에 "수주 목록"이라는 이름의 시트 생성
		Row o_row = null; // 행
		Cell o_cell = null; // 셀
		int o_rowNo = 0; // 첫 번째 행 번호 지정(0부터 시작)

		Font o_font = o_wb.createFont(); //글꼴 스타일
		o_font.setBold(true); // 글꼴 폰트 굵게
		...
		o_bodyStyle.setBorderTop(BorderStyle.THIN); // 셀 위 테두리 실선 적용
		o_bodyStyle.setBorderBottom(BorderStyle.THIN); // 셀 아래 테두리 실선 적용
		o_bodyStyle.setBorderLeft(BorderStyle.THIN); // 셀 왼쪽 테두리 실선 적용
		o_bodyStyle.setBorderRight(BorderStyle.THIN); // 셀 오른쪽 테두리 실선 적용
		o_bodyStyle.setAlignment(HorizontalAlignment.CENTER); // 가로 가운데 정렬
		o_bodyStyle.setVerticalAlignment(VerticalAlignment.CENTER); // 세로 가운데 정렬

		// 헤더 행 생성
		String[] o_arr = { "거래번호", "지점명", "재료명", "수주량", "단가", "총금액", "수주일시", "출하여부", "발주적요", "수주적요" };
		o_row = o_sheet.createRow(o_rowNo++); // 첫 번째 행 생성
		for (int i = 0; i < o_arr.length; i++) { // 헤더에 해당하는 열을 생성
			o_cell = o_row.createCell(i); // 각 셀을 생성
			o_cell.setCellStyle(o_headStyle); // 해더 스타일 적용
			o_cell.setCellValue(o_arr[i].toString()); // 셀 값 설정
		}

		// 데이터 부분 생성
		for (OrderDTO orderDTO : orderList) { // 각 주문에 대해 데이터를 삽입
			// 주문 번호
			o_row = o_sheet.createRow(o_rowNo++); // 새로운 행 생성
			o_sheet.autoSizeColumn(0); // 첫 번째 열의 너비 자동 조절
			o_sheet.setColumnWidth(0, (o_sheet.getColumnWidth(0)) + 1024); // 첫 번째 열 너비 추가 조정
			o_cell = o_row.createCell(0); // 첫 번째 열에 셀 생성
			o_cell.setCellStyle(o_bodyStyle); // 데이터 셀 스타일 적용
			o_cell.setCellValue(orderDTO.getOd_num()); // 주문 번호 값 설정
		...
		// 엑셀 파일 생성
		response.setContentType("application/vnd.ms-excel"); // 엑셀 파일 MIME 타입
		response.setHeader("Content-Disposition", "attachment;filename=orderList.xlsx"); // 다운로드 파일 이름 지정
		o_wb.write(response.getOutputStream()); // 워크북을 출력 스트림에 기록 (파일 다운로드)
		o_wb.close(); // 워크북 닫기
	}
